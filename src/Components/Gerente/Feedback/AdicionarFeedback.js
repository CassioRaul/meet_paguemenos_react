  ...
  return (
    ...
    <Box onSubmit={handleAddSubmit} component="form" noValidate autoComplete="off">
      ...
      {gerentes.filter(gerente => gerente.manager_id == idGerentes).map(gerente => {
        return (
          <TextField key={gerente.manager_id} sx={{ m: 1, width: '10%' }} type="text" name='feedback_manager_id' className="from__input" id="inputGroup-sizing-default" defaultValue={gerente.manager_id}  label="ID do Gerente" placeholder="ID do Gerente" multiline disabled/>
        )
      })}
      ...
      <TextField sx={{ m: 1, width: '15%' }} type="date" name='feedback_date' defaultValue={IdSchedule.schedule_date} className="from__input" id="inputGroup-sizing-default" label="Data" InputLabelProps={{ shrink: true, }} disabled/>

      <TextField sx={{ m: 1, width: '15%' }} type="time" name='feedback_hour' defaultValue={IdSchedule.schedule_hour} className="from__input" id="inputGroup-sizing-default" label="Hora" InputLabelProps={{ shrink: true, }} disabled/>
