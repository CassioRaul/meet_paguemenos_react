...
// Componente funcional para adicionar feedback
const AdicionarFeedbackC = ({ IdSchedule, handleCancelButton }) => {
  // Estado local para controlar a exibição do formulário de feedback
  const [showFeedbackForm, setShowFeedbackForm] = useState(false);
  // Uso de hooks personalizados para obter dados
  ...
  // Manipulador de envio de formulário de feedback
  const handleAddSubmit = (e) => {
    ...
    // Edita o agendamento relacionado
    editschedule(schedule.schedule_id, e.target)
      .then(res => {
        setSchedules([res])
      })
    // Chama a função para cancelar o formulário
  ...
  return (
    <div className="container_white">
      <Box onSubmit={handleAddSubmit} component="form" noValidate autoComplete="off">
        ...
        <input type='hidden' name='schedule_status_manager' defaultValue={IdSchedule.schedule_status_manager} />
        <input type='hidden' name='schedule_status_collaborator' defaultValue="FINALIZADA" />
        <br></br>
